...
- 失物招领填写必须登录，所有登录角色初次发布后都立即成为 `visible + approved`。
...
- `GET /api/user/lost-found`：读取 `visible + approved` 的寻物与招领启事；未登录可浏览。
- `POST /api/user/lost-found`：必须登录；初次发布后立即创建为 `visible + approved`，以登录身份发布，不进入审核队列；若该内容后来被管理端退回，`review_hold` 仍阻止作者编辑后自行重新公开。
- 公共校园墙混排列表默认仍过滤失物招领；话题目录、标签页和详情允许未登录查看。
- 不能通过校园动态发布器直接打失物招领标签，必须走专用表单。
