...
- 登录页主按钮为飞书登录；电脑打开官方授权页扫码，手机一般跳转飞书 App。
- 也可用用户名密码注册。提交后账号进入待审，审核员在「用户与权限」通过后才能登录。
- 审核员、管理员和超级管理员使用 `/admin/login` 的用户名与密码。
- 导航栏根据会话显示登录入口或个人中心入口。
- 个人中心支持资料、头像、邮箱验证与通知开关、连接飞书账户、发布、评论、收藏和通知；飞书账号不提供改密表单。
