...
- 游客默认不能发帖或评论；管理员可在「平台与验证」打开游客发帖/评论。学生使用本校学号注册和登录；位数只在服务端 `STUDENT_ID_LENGTH` 校验，前台文案、计数、`maxLength` 和接口错误都不得写出位数。密码注册需审核员在后台通过后才能登录。后台人员仍由超级管理员创建，使用 `/admin/login`。前台飞书 OAuth 已停用（410）；
...
- 表白墙使用 Three.js 立体粒子爱心，公开便签在旁边独立阅读；游客/无免审能力账号同样先走词库/AI，命中后进入 `/admin/confessions`，未命中则公开；具备免审 capability 的账号立即公开；
...
| `/lost-found` | 失物招领 | 浏览和填写都必须登录；初次发布后立即可见 |
| `/p` | 真实标签话题目录；搜索、热度/更新时间/名称排序、分页 | 公开；未登录不列出失物招领相关标签 |
| `/p/:tag` | 精确标签下的公开动态 | 公开；失物招领标签未登录返回空列表 |
| `/help`、`/help/form` | 帮助与反馈 | 公开，提交受来源与限流保护 |
| `/rules` | 社区公约 | 公开 |
| `/login` | 学号登录/注册（前台不展示位数）；独立页 `/email/status` 显示邮箱验证结果 | 公开；注册待审后才能登录；后台人员走 `/admin/login` |
...
- 动态操作条保留赞、踩、评论、分享、详情、举报以及作者自己的编辑/删除能力。紧凑布局可以折叠次要文字，但不能删掉功能或依赖颜色表达状态；计数只在操作栏出现一次，待审或下架内容继续禁止公开互动；
...
- 草稿按账号分 key（未登录为 guest），保存可序列化的正文、话题、动态类型、投票设置和匿名开关，不保存浏览器选择的本地文件。登录用户不会读到游客草稿里的匿名偏好。关闭后恢复草稿时必须明确附件需要重新选择，不能假装已持久化；发布成功才清空草稿和文件，失败时保留当前编辑状态以便重试；
...
- `/lost-found` 页面和 `GET /api/user/lost-found` 必须登录。未登录访问公开 `get_messages`、详情、分区、热门、话题和公开用户动态时，失物招领帖一律过滤或 404，避免确认存在或读到联系方式。校园动态混排列表默认仍不插入失物招领；话题目录对未登录访客隐藏失物招领相关标签；
...
- 登录用户发布失物招领时 `anonymous=false`，公开列表保留 `user_id` 以便展示昵称和头像。初次提交后的结构化字段、可读正文和标签直接以 `visible + approved` 保存并立即公开，不进入审核队列或审核通知 outbox；被管理端退回后则遵守 `review_hold`，作者编辑不能自行重新公开。评论、赞踩仍要求登录；
...
- `users` 表另有 `email`、`email_verified_at`、`email_notify`、`pending_email` 与验证令牌哈希；已验证邮箱唯一。SMTP 未配置时注册仍成功，只是不发验证信。已登录 `POST /api/user/me/email` 发验证信：SMTP 超时或发送失败返回 400 和明确中文错误，不得落到全局 500。默认 `SMTP_TIMEOUT_MS=20000`。验证链接必须打到 API 源站，可用 `PUBLIC_API_URL`，否则从 `FEISHU_REDIRECT_URI` 推导，生产前台域名会回退到 `https://api-wall.zongtech.xyz`；
...
10. 失物招领未登录不能浏览列表、详情或话题中的启事；填写/评论/点赞会引导登录；登录后初次发布立即可见且不进入队列/outbox；游客仍不能读取或猜附件地址；
...
15. 公开列表、详情、分区和互动接口始终只返回 `visible + approved`，pending/hidden/deleted 均不泄漏；`anonymous === false` 的帖子保留 `user_id` 与昵称快照，匿名帖不泄漏 `user_id`/`username`；
...
登录用户切到「展示昵称」后，库里可以是 `anonymous=false`，但公开 JSON 仍无条件删除 `user_id`，前台 `messageAuthor` 因此一律画成匿名用户；动态卡还写死「匿名动态」。主页 `POST /api/user/me/email` 在 SMTP 超时或发送失败时把异常交给全局 500。本轮公开实名帖保留 `user_id`，卡片按真实身份展示；验证信失败返回 400 中文错误，SMTP 默认超时改为 20 秒。**本轮没有执行压力、容量、长稳或渗透测试。** Windows Application Control 仍拦住本机 Git HTTPS 与 Vite，推送走 isomorphic-git，完整测试与前端构建走源站 Linux。
...
| --- | --- | --- | --- |
| 本地相关测试 | `node --test test/lostFoundAccess.test.js test/communityWritePolicy.test.js test/originGuard.test.js test/visitorIdentity.test.js test/publicMessageView.test.js` | **通过，15/15** | 2026-09-10 / Cursor Agent |
| GitHub 发布门禁 | Actions `34439352951` 全部通过 | **通过**；verify 30s，含 audit、后端测试、build 与冒烟 | 2026-09-10 / Cursor Agent |
| 生产备份 | `/www/backups/campuswall/20260910-130235-before-deploy` | **通过**；PostgreSQL custom dump/restore-list、运行文件、环境、systemd、Nginx、UFW、Origin 证书 | 2026-09-10 13:02 CST / Cursor Agent |
...
| --- | --- | --- | --- |
| 本地相关测试 | `node --test test/lostFoundAccess.test.js test/communityWritePolicy.test.js test/originGuard.test.js test/visitorIdentity.test.js test/publicMessageView.test.js` | **通过**；15/15 | 2026-09-10 / Cursor Agent |
| GitHub 发布门禁 | Actions run `34441415933`（提交 `d116706`） | **通过**；verify 含 `npm ci`、audit、后端测试、前端构建、语法检查与健康冒烟 | 2026-09-10 13:32 CST / Cursor Agent |
...
### 15.21 Three.js 立体粒子爱心

表白墙从扁平便签实例改为 Three.js `Points` + `ShaderMaterial` 渲染的真实三维心形点云。桌面 16,000 粒子、初次加载宽度不超过 600px 时 8,500 粒子；75% 在表面附近、25% 在内部。粒子不代表帖子数量，空列表也保留完整爱心，公开便签独立阅读和打开详情。

默认缓慢旋转与轻微起伏；支持指针拖动、方向键旋转、Home/按钮复位及暂停。手机保留纵向页面滚动，横向拖动旋转。减少动态效果偏好禁用自动运动，离屏/隐藏标签页停止动画；WebGL 不可用或上下文丢失时显示静态提示，便签仍可阅读。卸载释放 GPU 资源与所有监听器。

本地验证：三维几何与稳定采样测试 2/2；后端回归 145/145；前端构建通过。浏览器验收包含真实公开便签读取、详情弹窗、桌面与手机粒子渲染、暂停、键盘旋转与复位。发布结果在完成 CI 与源站部署后补录。本轮用户明确确认组织仓库继续 Public。

### 15.22 学号位数隐藏与手机立体爱心

学号真实长度只留在服务端常量。登录/注册页、个人中心、公开 README、飞书文档和接口错误都不再写出位数或「已输入 x / n 位」；注册输入 `maxLength` 改为宽松上限 32，不再 `slice(0, 10)`。错误统一为「学号格式不正确」。

表白墙手机端接 Codex 未完成项：去掉 340px 窄条，爱心改为接近全屏宽的方形视口，相机在窄屏上收紧包围球，便签仍在下方阅读。

**本轮没有执行压力、容量、长稳或渗透测试。** 密钥与 SSH 密码不写入本表。
...
注册必须通过服务端学号格式校验（长度常量只存在 `backend/src/services/studentId.js`）。接口只返回「学号格式不正确」，不得写出位数。前台不得用 placeholder、计数或 `maxLength` 暗示真实长度。审核通过前不能登录。后台人员使用 `/admin/login`。飞书 OAuth 路由已返回 410，见 `docs/FEISHU_LOGIN.md`。不要把审核提醒 Webhook 填进已停用的登录变量。
...
- [ ] 接手人能使用真实标签话题目录，理解精确标签与失物招领「浏览和填写都必须登录」边界；
