...
export const config = {
  ...
  postImageMaxEdge: intEnv('POST_IMAGE_MAX_EDGE', 2048, { min: 640, max: 8192 }),
  postImageWebpQuality: intEnv('POST_IMAGE_WEBP_QUALITY', 80, { min: 40, max: 95 }),
  postImageMaxOutputBytes: intEnv('POST_IMAGE_MAX_OUTPUT_BYTES', 1572864, { min: 131072, max: 20 * 1024 * 1024 }),
  maxPostImageInputPixels: intEnv('MAX_POST_IMAGE_INPUT_PIXELS', 50_000_000, { min: 1_000_000, max: 150_000_000 }),
  ...
  rateLimitEmail: intEnv('RATE_LIMIT_EMAIL', 8, { min: 1, max: 100 }),
  ...
  captchaProtectLogin: boolEnv('CAPTCHA_PROTECT_LOGIN', true),
  captchaProtectRegister: boolEnv('CAPTCHA_PROTECT_REGISTER', true),
  captchaProtectAdminLogin: boolEnv('CAPTCHA_PROTECT_ADMIN_LOGIN', true),
  captchaAllowedHostnames: listEnv('CAPTCHA_ALLOWED_HOSTNAMES', []),
  ...
  moderationNotifyEmailTo: String(process.env.MODERATION_NOTIFY_EMAIL_TO || '').trim(),
  smtpHost: String(process.env.SMTP_HOST || '').trim(),
  smtpPort: intEnv('SMTP_PORT', 587, { min: 1, max: 65535 }),
  smtpSecure: boolEnv('SMTP_SECURE', String(process.env.SMTP_PORT || '587') === '465'),
